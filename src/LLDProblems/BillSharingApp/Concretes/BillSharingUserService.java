...
public class BillSharingUserService implements IBillSharingUserService {
  ...
  public void showUserBillInfo() {
    ...
    for (Map.Entry<String, UserEntity> kv : userEntityMap.entrySet()) {
      ...
      for (Map.Entry<UserEntity, Double> mappedUserKV :
          kv.getValue().getMappedUserEntityList().entrySet()) {
        // double diff=mappedUserKV.getValue()- .;
        double diff =
            amountPaid(kv.getKey(), mappedUserKV.getKey().getName())
        ...
        if (diff != 0.0) {
          if (diff > 0) {
            ...
            System.out.println(
                "You owe Rs " + Math.abs(diff) + " from " + mappedUserKV.getKey().getName());
          }
        } else {
