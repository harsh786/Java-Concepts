...
public class SimpleTicketCalculationStartegy implements ITicketCalculationStartegy {
  ...
  public double calculate(Ticket ticket) {
    ...
    IParkingSlotType parkingSlotType =
        ParkingSlotTypeFactory.getParkingSlot(
            ticket.getAllotedParkingSlot().getEnumParkingSpotType());
    if (diffInMin % 60 > 1) {
      ticketCost =
          parkingSlotType.getParkingSpotMinPrice()
              + ((diffInMin / 60) * parkingSlotType.getParkingSpotPerHourPrice());
    } else {
